...
const INTEGRATIONS = [
  ...
  { name: 'Kubernetes', cat: 'Containers', icon: '⎈', color: '#326ce5', status: 'Native', badge: 'Popular', desc: 'Audit logs · RBAC analysis · Pod security events' },
  { name: 'Docker', cat: 'Containers', icon: '🐳', color: '#2496ed', status: 'Native', badge: '', desc: 'Container lifecycle · Registry scan events' },
  { name: 'GitHub', cat: 'DevOps & IaC', icon: '🐙', color: '#f0f0f0', status: 'Native', badge: '', desc: 'Code scanning · Secret detect · GHAS alerts' },
  { name: 'Terraform', cat: 'DevOps & IaC', icon: '🏗', color: '#7b42bc', status: 'Native', badge: '', desc: 'Drift detection · IaC mis-config scanning' },
];
  ...
  return (
    <MarketingLayout>
      ...
      <section style={{ padding: '96px 48px 72px', maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 64, fontWeight: 900, color: '#fff', lineHeight: 1.08, letterSpacing: -2, marginBottom: 20, maxWidth: 780, margin: '0 auto 20px' }}>
        ...
        <div style={{ maxWidth: 440, margin: '0 auto', position: 'relative' }}>
          ...
          <input placeholder="Search integrations…" value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 16px 12px 42px', color: '#fff', fontSize: 14, fontFamily: 'Inter,sans-serif', outline: 'none' }}
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontFamily: 'Outfit,sans-serif', fontSize: 16, fontWeight: 700, color: '#fff' }}>{int.name}</span>
                  </div>
      ...
      <section style={{ background: 'var(--bg-secondary)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '72px 48px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'center' }}>
          <div>
            <h2 style={{ fontFamily: 'Outfit,sans-serif', fontSize: 40, fontWeight: 900, color: '#fff', lineHeight: 1.15, marginBottom: 16 }}>Connect your first source in under 10 minutes</h2>
            ...
            {[['1', 'Install the CLI', 'npm install -g @sentinelx/cli && sentinelx login'], ['2', 'Connect AWS', 'sentinelx integrate aws --account-id 123456789012'], ['3', 'Verify & monitor', 'sentinelx status  →  Events/hr: 142,830  ✓']].map(([n, t, d]) => (
              <div key={n} style={{ display: 'flex', gap: 14, marginBottom: 20 }}>
                ...
                <div><div style={{ fontWeight: 700, color: '#fff', marginBottom: 4, fontSize: 14 }}>{t}</div><code style={{ fontFamily: 'JetBrains Mono', fontSize: 11.5, color: '#2dd4bf', background: 'rgba(0,229,255,0.06)', padding: '3px 8px', borderRadius: 5 }}>{d}</code></div>
              </div>
            ))}
      ...
      <section style={{ padding: '72px 48px', maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
        <h2 style={{ fontFamily: 'Outfit,sans-serif', fontSize: 38, fontWeight: 900, color: '#fff', marginBottom: 14 }}>Don't see your tool?</h2>
