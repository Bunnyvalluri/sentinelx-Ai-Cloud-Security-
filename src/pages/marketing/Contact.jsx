  ...
  return (
    <MarketingLayout>
      <section style={{ padding: '100px 48px 60px', maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: 'Outfit,sans-serif', fontSize: 52, fontWeight: 900, color: 'var(--text-primary)', lineHeight: 1.1, marginBottom: 18, letterSpacing: -1 }}>We'd love to hear from you</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 56 }}>
            {INQUIRIES.map(inq => (
              ...
              >
                ...
                <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: 15, marginBottom: 4 }}>{inq.type}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {OFFICES.map(o => (
              <div key={o.city} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '20px 22px' }}>
                ...
                <div style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: 15, marginBottom: 8 }}>{o.city}</div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: '36px', position: 'sticky', top: 88, height: 'fit-content' }}>
          {submitted ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              ...
              <h2 style={{ fontFamily: 'Outfit,sans-serif', fontSize: 26, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 12 }}>Message sent!</h2>
            ...
            <>
              <h2 style={{ fontFamily: 'Outfit,sans-serif', fontSize: 22, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 24 }}>Send us a message</h2>
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input required type={f.type} placeholder={f.placeholder} value={form[f.key]} onChange={e => setForm(p => ({ ...p, [f.key]: e.target.value }))}
                      style={{ width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '11px 14px', color: 'var(--text-primary)', fontSize: 14, fontFamily: 'Inter,sans-serif', outline: 'none', boxSizing: 'border-box' }}
                ...
                <div>
                  ...
                  <select value={form.inquiry} onChange={e => setForm(p => ({ ...p, inquiry: e.target.value }))} style={{ width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '11px 14px', color: 'var(--text-primary)', fontSize: 14, fontFamily: 'Inter,sans-serif', outline: 'none', appearance: 'none', cursor: 'pointer' }}>
                ...
                <div>
                  ...
                  <textarea required placeholder="Tell us about your security environment, team size, and what you're trying to solve..." value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={5}
                    style={{ width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '11px 14px', color: 'var(--text-primary)', fontSize: 14, fontFamily: 'Inter,sans-serif', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }}
                    onFocus={e => e.target.style.borderColor = 'rgba(16, 185, 129,0.5)'}
                    onBlur={e => e.target.style.borderColor = 'var(--border)'}
                  />
                </div>
